...
public class MultiImageSelectorFragment extends Fragment {
	...
	public static final String FOLODER = Environment.getExternalStorageDirectory()+"/com.union.ineighbor/";
	...
	private void showCameraAction() {
		if (ContextCompat.checkSelfPermission(getActivity(), Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED ||  ActivityCompat.checkSelfPermission((getActivity()), Manifest.permission.WRITE_EXTERNAL_STORAGE)
			...
			if (intent.resolveActivity(getActivity().getPackageManager()) != null) {
				...
				if (mTmpFile != null && mTmpFile.exists()) {
					if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.N){
						...
						Uri contentUri = FileProvider.getUriForFile(getActivity(), "com.union.huayu.fileprovider", mTmpFile);
			...
			if (mode == MODE_MULTI) {
				if (resultList.contains(image.path)) {
					...
					if (selectImageCount() == resultList.size()) {
						Toast.makeText(getActivity(), "你最多只能选择"+selectImageCount()+"张图片", Toast.LENGTH_SHORT).show();
						return;
					}
	...
	private int selectImageCount() {
		return getArguments() == null ? 6 : getArguments().getInt(EXTRA_SELECT_COUNT);
	}
